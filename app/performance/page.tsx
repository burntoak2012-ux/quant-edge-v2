  ...
  return (
    <main className="qe-grid min-h-screen px-5 py-8 text-white sm:px-10">
      <div className="mx-auto max-w-6xl">
        ...
        <header className="mt-6 border-b border-cyan-100/10 pb-6">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-lime-300">Model lab</p>
          <h1 className="mt-2 text-4xl font-bold">Performance</h1>
          <p className="mt-3 max-w-2xl text-slate-400">Private model tracking from saved predictions. Results become meaningful as fixtures settle over time.</p>
        </header>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {[
            ...
            ["Accuracy", percentage(settled.length ? Math.round((correct / settled.length) * 100) : null)],
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
          <section>
            ...
            <h2 className="mt-2 text-2xl font-bold">Accuracy by probability band</h2>
            <div className="qe-panel mt-5 overflow-hidden rounded-2xl border">
              {bands.map((band) => (
                <div className="border-b border-slate-800 p-4 last:border-b-0" key={band.label}>
                  ...
                  <p className="mt-2 text-xs text-slate-500">Observed accuracy: {percentage(band.accuracy)}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-lime-300">Market check</p>
            <h2 className="mt-2 text-2xl font-bold">Potential value</h2>
            <div className="qe-panel mt-5 rounded-2xl border p-5">
              <p className="text-sm text-slate-400">Settled predictions with at least +5 percentage points of model-versus-market value.</p>
              <p className="mt-5 text-4xl font-bold text-lime-300">{valueRows.length ? `${Math.round((valueCorrect / valueRows.length) * 100)}%` : "-"}</p>
              <p className="mt-2 text-sm text-slate-500">{valueCorrect} correct from {valueRows.length} potential-value predictions</p>
              <p className="mt-5 border-t border-slate-800 pt-4 text-xs text-slate-500">This is accuracy, not ROI. Profitability requires settled outcome odds and staking rules.</p>
            </div>
          </section>
        </div>
      </div>
